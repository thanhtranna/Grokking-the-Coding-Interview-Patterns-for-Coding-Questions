   ...
   static vector<int> findWordConcatenation(const string &str,
      ...
      for (int i = 0; i <= str.length() - wordsCount * wordLength + 1; i++) {
         ...
         for (int j = 0; j < wordsCount; j++) {
            ...
            // no need to process further if the word has higher frequency than
            // required
            ...
            if (j + 1 ==
                wordsCount) {  // store index if we have found all the words
...
int main(int argc, char *argv[]) {
   vector<int> result = WordConcatenation::findWordConcatenation(
       "catfoxcat", vector<string>{"cat", "fox"});
   for (auto num : result) {
      cout << num << " ";
   }
   cout << endl;
   ...
   result = WordConcatenation::findWordConcatenation(
       "catcatfoxfox", vector<string>{"cat", "fox"});
   for (auto num : result) {
      cout << num << " ";
   }
   cout << endl;
